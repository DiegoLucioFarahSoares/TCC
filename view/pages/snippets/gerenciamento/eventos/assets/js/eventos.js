let Eventos = function () {
    ...
    this.view = (name, width, item) => {
        return new Promise((resolve) => {
            $.ajax({
                url: '/view/pages/snippets/gerenciamento/eventos/' + name + '?view=' + Math.random() * 1000,
                beforesend: function () {
                    jDialog.openLoading('Carregando....');
                },
    ...
    this.consultar = function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.status){
                    let template = $('#templateEventos').html();
                    let html = ejs.compile(template)({dados: response.resultSet});

                    $('#resultadoEventos').html(html);
                    $('#resultadoEventos table').DataTable({
                        language: {
    ...
    this.salvar = function () {
        ...
        const formData = new FormData(document.querySelector('#formNovosEventos'));
     ...
     * @param nome
     */
    this.viewEditar = (idEventos, idPessoa, nome, descricao, horas, dataInicio, dataFim) => {
        jDialog.openLoading('Carregando....');
        ...
        self.view('modal-editar.php', '80%', 0).then((result) => {
            ...
            $('#formEditarEventos #idEventos').val(idEventos);
            $('#formEditarEventos #iPessoa').val(idPessoa).selectpicker("refresh");
            $('#formEditarEventos #iNome').val(nome);
            $('#formEditarEventos #iDescricao').val(descricao);
            $('#formEditarEventos #iHoras').val(horas);
            $('#formEditarEventos #iDataInicio').val(dataInicio);
            $('#formEditarEventos #iDataFim').val(dataFim);
    ...
    this.editar = function () {
        ...
        const formData = new FormData(document.querySelector('#formEditarEventos'));

        $.ajax({
            method : 'POST',
            type   : 'POST',
            url    : '/api/eventos/editar',
            data        :   formData,
            cache       :   false,
            contentType :   false,
            enctype     :   'multipart/form-data',
     ...
     * Excluir
     */
    this.excluir = function (idEventos) {
        Swal.fire({
            title: 'Você tem certeza que deseja realizar essa operação?',
            text: "",
            icon: 'warning',
     ...
     * @param idEventos
     */
    this.status = function(idEventos) {
        $.ajax({
            method : 'GET',
            type   : 'GET',
            url:  '/api/eventos/status',
            data: {
                idEventos: idEventos,
                status: ($('#status-'+ idEventos +' input[type="checkbox"]').prop('checked')) ? 1 : 2
            },
            beforeSend: function () {
                jDialog.openLoading('Carregando....');
            },
...
$(document).ready(function() {
    $(document).on('click', '#formNovosEventos #btnSalvar', function () {
        _eventos.formValidate();
    });
});